# -*- coding: utf-8 -*-
"""
The module :mod:`bag.tests.form` provides an implementation of a client form
...
import bag
from bag.models import BaseModel
from bag.fields import Command
from bag.tools.safe_eval import safe_eval
...
class Form:
    ...
    * properly handle defaults & onchanges around x2many fields.
    ...
    of :class:`~bag.fields.Many2one` fields, one can assign a recordset::
    ...
    For :class:`~bag.fields.Many2many` fields, the field itself is a
    :class:`~bag.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~bag.fields.One2many` are reified as :class:`~O2MProxy`.

    Because the :class:`~bag.fields.One2many` only exists through its parent,
    ...
    @classmethod
    def from_action(cls, env: bag.api.Environment, action: dict) -> Form:
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~bag.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~bag.fields.One2many` record.
...
def convert_read_to_form(values, fields):
    ...
    for fname, value in values.items():
        ...
        if field_info['type'] == 'one2many':
            ...
            value = bag.fields.Datetime.to_string(value)
        elif field_info['type'] == 'date' and isinstance(value, date):
            value = bag.fields.Date.to_string(value)
        result[fname] = value
    return result
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return bag.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return bag.fields.Date.to_string(value)
    return value
