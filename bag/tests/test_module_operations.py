...
import bag
from bag.tools import config, topological_sort, unique
from bag.modules.registry import Registry
from bag.netsvc import init_logger
from bag.tests import standalone_tests
import bag.tests.loader

_logger = logging.getLogger('bag.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Bag modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Bag modules")
...
def test_standalone(args):
    ...
    for module_name in registry._init_modules:
        ...
        bag.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        bag.tools.config['addons_path'] = ','.join([args.addons_path, bag.tools.config['addons_path']])
        if args.data_dir:
            bag.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'bag.modules.loading': {'level': 'CRITICAL'},
            'bag.sql_db': {'level': 'CRITICAL'},
            'bag.models.unlink': {'level': 'WARNING'},
            'bag.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
