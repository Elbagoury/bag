...
from .suite import BagSuite
from .result import BagTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'bag.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"bag.upgrade.{module}",
        f"bag.addons.{module}.migrations",
        f"bag.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return BagSuite(sorted(tests, key=lambda t: t.test_sequence))
