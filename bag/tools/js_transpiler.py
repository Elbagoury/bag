"""
This code is what let us use ES6-style modules in bag.
Classic Bag modules are composed of a top-level :samp:`bag.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@bag-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** bag-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual bag one with minimal changes.
...
from bag.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom bag modules.
    ...
    legacy_bag_define = get_aliased_bag_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_bag_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_bag_define:
        content += legacy_bag_define
    return content
...
def url_to_module_path(url):
    """
    Bag modules each have a name. (bag.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_bag_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the bag.define call.
    ...
    """
    return f"""bag.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
ODOO_MODULE_RE = re.compile(r"""
    ...
    @bag-module                    # '@bag-module' statement
...
def is_bag_module(url, content):
    """
    Detect if the file is a native bag module.
    We look for a comment containing @bag-module.

    :param url:
    :param content: source code
    :return: is this a bag module that need transpilation ?
    """
...
def get_aliased_bag_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the bag source base being:
    ...
    > bag.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @bag-module */
        // after
        /** @bag-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @bag-module */
        // after
        /** @bag-module alias=web.AbstractAction default=false */
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\nbag.define(`%s`, ['%s'], function (require) {
                        return require('%s');
