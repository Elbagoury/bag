...
from bag import fields, http, tools, _
from bag.addons.website.controllers.main import QueryURL
from bag.addons.website.models.ir_http import sitemap_qs2dom
from bag.addons.website_profile.controllers.main import WebsiteProfile
from bag.exceptions import AccessError, ValidationError, UserError, MissingError
from bag.http import request, Response
from bag.osv import expression
from bag.tools import consteq, email_split
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if not slide.channel_id.is_member:
            if not isinstance(request.session.get('viewed_slides'), dict):
                # Compatibility layer with Bag 15.0,
                # where `viewed_slides` are stored as `list` in sessions.
                # For performance concerns, `viewed_slides` is changed to a dict,
                # but sessions coming from Bag 15.0 after an upgrade should still be compatible.
                # This compatibility layer regarding `viewed_slides` must remain from Bag 16.0 and above,
                # as this is possible to do a jump of multiple versions in one go,
                # and carry the sessions with the upgrade.
                # e.g. upgrade from Bag 15.0 to 18.0.
