...
from bag import fields, http
from bag.addons.base.tests.common import HttpCaseWithUserDemo, HttpCaseWithUserPortal
from bag.addons.mail.tests.common import mail_new_test_user
from bag.addons.website_event.tests.common import TestEventOnlineCommon, OnlineEventCase
...
class TestEventRegisterUTM(HttpCase, TestEventOnlineCommon):
    def test_event_registration_utm_values(self):
        ...
        self.opener.cookies.update({
            'bag_utm_campaign': event_campaign.name,
            'bag_utm_source': self.env.ref('utm.utm_source_newsletter').name,
            'bag_utm_medium': self.env.ref('utm.utm_medium_email').name
        })
