...
import { describe, dryRun, globals, start, stop } from "@bag/hoot";
import { Deferred, microTick } from "@bag/hoot-dom";
import { watchKeys, watchListeners } from "@bag/hoot-mock";
import { whenReady } from "@bag/owl";
...
class ModuleSetLoader extends loader.constructor {
    ...
    constructor(moduleSet) {
        ...
        bag.define = this.define.bind(this);
        bag.loader = this;
    }
...
const AUTO_INCLUDED_ADDONS = {
    /**
     * spreadsheet addons defines a module that does not starts with `@spreadsheet` but `@bag` (`@bag/o-spreadsheet)
     * To ensure that this module is loaded, we have to include `bag` in the dependencies
     */
    spreadsheet: ["bag"],
    /**
...
const CSRF_TOKEN = bag.csrf_token;
...
const R_DEFAULT_MODULE = /^@bag\/(owl|hoot)/;
