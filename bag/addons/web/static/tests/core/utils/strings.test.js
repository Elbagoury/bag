...
test("escape", () => {
    ...
    expect(escape(`<a href="https://www.bag.com">bag<a>`)).toBe(
        `&lt;a href=&quot;https://www.bag.com&quot;&gt;bag&lt;a&gt;`
    );
    expect(escape(`<a href='https://www.bag.com'>bag<a>`)).toBe(
        `&lt;a href=&#x27;https://www.bag.com&#x27;&gt;bag&lt;a&gt;`
    );
    expect(escape("<a href='https://www.bag.com'>Bag`s website<a>")).toBe(
        `&lt;a href=&#x27;https://www.bag.com&#x27;&gt;Bag&#x60;s website&lt;a&gt;`
    );
});
...
test("escapeRegExp", () => {
    ...
    expect(escapeRegExp("[wowl]")).toBe("\\[wowl\\]");
    expect(escapeRegExp("[wowl.bag]")).toBe("\\[wowl\\.bag\\]");
    expect(escapeRegExp("^bag.define([.]*)$")).toBe("\\^bag\\.define\\(\\[\\.\\]\\*\\)\\$");
    expect(escapeRegExp("[.*+?^${}()|[]\\")).toBe("\\[\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
});
...
test("isEmail", () => {
    ...
    expect(isEmail("test@bag")).toBe(false);
    expect(isEmail("test@bag@bag.com")).toBe(false);
    expect(isEmail("te st@bag.com")).toBe(false);

    expect(isEmail("test@bag.com")).toBe(true);
});
