...
function BagEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
BagEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
BagEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Bag where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new BagEvent(this, name, info);
