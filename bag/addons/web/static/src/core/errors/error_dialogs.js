...
import { Component, useRef, useState, markup } from "@bag/owl";
...
export const bagExceptionTitleMap = new Map(
    Object.entries({
        "bag.addons.base.models.ir_mail_server.MailDeliveryException": _t("MailDeliveryException"),
        "bag.exceptions.AccessDenied": _t("Access Denied"),
        "bag.exceptions.MissingError": _t("Missing Record"),
        "bag.addons.web.controllers.action.MissingActionError": _t("Missing Action"),
        "bag.exceptions.UserError": _t("Invalid Operation"),
        "bag.exceptions.ValidationError": _t("Validation Error"),
        "bag.exceptions.AccessError": _t("Access Error"),
        "bag.exceptions.Warning": _t("Warning"),
    })
);
...
ClientErrorDialog.title = _t("Bag Client Error");

// -----------------------------------------------------------------------------
// Network Error Dialog
// -----------------------------------------------------------------------------
export class NetworkErrorDialog extends ErrorDialog {}
NetworkErrorDialog.title = _t("Bag Network Error");
...
export class RPCErrorDialog extends ErrorDialog {
    ...
    inferTitle() {
        ...
        if (this.props.exceptionName && bagExceptionTitleMap.has(this.props.exceptionName)) {
            this.title = bagExceptionTitleMap.get(this.props.exceptionName).toString();
            return;
        }
        ...
        switch (this.props.type) {
            case "server":
                this.title = _t("Bag Server Error");
                break;
            case "script":
                this.title = _t("Bag Client Error");
                break;
            case "network":
                this.title = _t("Bag Network Error");
                break;
        }
    }
...
export class WarningDialog extends Component {
    ...
    inferTitle() {
        if (this.props.exceptionName && bagExceptionTitleMap.has(this.props.exceptionName)) {
            return bagExceptionTitleMap.get(this.props.exceptionName).toString();
        }
        return this.props.title || _t("Bag Warning");
    }
}
...
export class RedirectWarningDialog extends Component {
    ...
    setup() {
        ...
        this.title = capitalize(subType) || _t("Bag Warning");
        this.message = message;
...
registry
    ...
    .add("bag.exceptions.AccessDenied", WarningDialog)
    .add("bag.exceptions.AccessError", WarningDialog)
    .add("bag.exceptions.MissingError", WarningDialog)
    .add("bag.addons.web.controllers.action.MissingActionError", WarningDialog)
    .add("bag.exceptions.UserError", WarningDialog)
    .add("bag.exceptions.ValidationError", WarningDialog)
    .add("bag.exceptions.RedirectWarning", RedirectWarningDialog)
    .add("bag.http.SessionExpiredException", SessionExpiredDialog)
