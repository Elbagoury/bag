...
export function stateToUrl(state) {
    ...
    const start_url = isScopedApp() ? "scoped_app" : "bag";
 ...
 * external website). Unfortunately, Bag wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/bag") &&
            (["/web", "/bag"].includes(url.pathname) || url.pathname.startsWith("/bag/")) &&
            ev.target.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/bag") && url.hash) {
