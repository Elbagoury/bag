...
try:
    ...
    from bag.tools._vendor.send_file import send_file

import bag
import bag.modules.registry
from bag import SUPERUSER_ID, _, http, api
from bag.addons.base.models.assetsbundle import ANY_UNIQUE
from bag.exceptions import AccessError, UserError
from bag.http import request, Response
from bag.tools import file_open, file_path, replace_exceptions, str2bool
from bag.tools.image import image_guess_size_from_field_name
from bag.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Bag is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Bag is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if bag.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=bag.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or bag.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the Bag logo instead", exc_info=True)
