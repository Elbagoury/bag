...
from bag.http import content_disposition, dispatch_rpc, request, Response
from bag.service import db
from bag.tools.misc import file_open, str2bool
from bag.tools.translate import _

from bag.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = bag.tools.config.verify_admin_password('admin')
        d['list_db'] = bag.tools.config['list_db']
        d['langs'] = bag.service.db.exp_list_lang()
        d['countries'] = bag.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = bag.service.db.list_db_incompatible(d['databases'])
        except bag.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = bag.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            bag.service.db.check_super(master_pwd)
            ...
            dump_stream = bag.service.db.dump_db(name, None, backup_format)
