...
import { getAdjacentPreviousSiblings, isBlock, rgbToHex, commonParentGet } from '../editor/bag-editor/src/utils/utils';
...
export async function toInline($editable, options) {
    $editable.removeClass('bag-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('bag-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const { fonts } = bag.loader.modules.get("@web_editor/js/wysiwyg/fonts");
