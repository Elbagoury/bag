...
export class LinkPopoverWidget {
    static createFor(params) {
        ...
        params.wysiwyg?.bagEditor.observerUnactive('LinkPopoverWidget');
        popoverWidget.start(); // This is not async
        params.wysiwyg?.bagEditor.observerActive('LinkPopoverWidget');
    ...
    start() {
        ...
        if (document !== this.wysiwyg.bagEditor.document) {
            $(this.wysiwyg.bagEditor.document).on('mouseup.link_popover', onClickDocument);
        }
    ...
    destroy() {
        ...
        $(this.wysiwyg.bagEditor.document).off('.link_popover');
