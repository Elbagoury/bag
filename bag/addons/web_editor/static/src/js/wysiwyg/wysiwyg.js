...
import * as BagEditorLib from "@web_editor/js/editor/bag-editor/src/BagEditor";
...
import { isIconElement, isSelectionInSelectors, peek } from '@web_editor/js/editor/bag-editor/src/utils/utils';
...
const BagEditor = BagEditorLib.BagEditor;
const getDeepRange = BagEditorLib.getDeepRange;
const getInSelection = BagEditorLib.getInSelection;
const isProtected = BagEditorLib.isProtected;
const rgbToHex = BagEditorLib.rgbToHex;
const preserveCursor = BagEditorLib.preserveCursor;
const closestElement = BagEditorLib.closestElement;
const setSelection = BagEditorLib.setSelection;
const endPos = BagEditorLib.endPos;
const hasValidSelection = BagEditorLib.hasValidSelection;
const parseHTML = BagEditorLib.parseHTML;
const closestBlock = BagEditorLib.closestBlock;
const getRangePosition = BagEditorLib.getRangePosition;
const childNodeIndex = BagEditorLib.childNodeIndex;
const fillEmpty = BagEditorLib.fillEmpty;
const isVisible = BagEditorLib.isVisible;
const getDeepestPosition = BagEditorLib.getDeepestPosition;
const paragraphRelatedElements = BagEditorLib.paragraphRelatedElements;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.bagEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.bagEditor.historyUnstash();
                ...
                // hides it).
                if (hasValidSelection(this.bagEditor.editable)) {
                    this.bagEditor.deselectTable();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.bagEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                    this.bagEditor._computeHistorySelection();
                } finally {
                    this.bagEditor.historyUnpauseSteps();
                }
            }
        };
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.bagEditor.withoutRollback(() => this.bagEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        this.bagEditor = new BagEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.bagEditor.addEventListener('contentChanged', function () {
        ...
        if (this.bagEditor.document !== document) {
            this.bagEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.bagEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.bagEditor.document.addEventListener("keyup", this._signalOnline, true);
        }
        ...
        if ($('wrapwrap').length) {
            this.multiSelectionTarget = $().getScrollingTarget(this.bagEditor.document);
            this.multiSelectionTarget.addEventListener('scroll', this.bagEditor.multiselectionRefresh, { passive: true });
        }
        ...
        this._observeBagFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.bagEditor.observerUnactive();
                const $field = $(this);
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.bagEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            ...
            $(this.bagEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.bagEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.bagEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.bagEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.bagEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.bagEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        if (!this.state.showSnippetsMenu) {
            this.setCSSVariables(this.toolbarEl);
        }

        this.bagEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.bagEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.bagEditor.isMobile) {
                this.bagEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.bagEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.bagEditor) {
            this.bagEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.bagEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.bagEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.bagEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.bagEditor.destroy();
        }
        ...
        this.multiSelectionTarget?.removeEventListener('scroll', this.bagEditor.multiselectionRefresh, { passive: true });
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper bag-editor">');
    ...
    getValue(options) {
        ...
        if (this.bagEditor) {
            this.bagEditor.cleanForSave($editable[0]);
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.bagEditor) : [];
     ...
     */
    setValue(value) {
        this.bagEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.bagEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.bagEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.bagEditor && !this.bagEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.bagEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    }
    getDeepRange() {
        return getDeepRange(this.bagEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.bagEditor.editable) {
        if (this.bagEditor) {
            this.bagEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.bagEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the Bag field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeBagFieldChanges() {
        ...
        if (this.bagFieldObservers) {
            for (let observerData of this.bagFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const bagFieldSelector = '[data-oe-model], [data-oe-translation-source-sha]';
            const $bagFields = this.$editable.find(bagFieldSelector);
            const renderingClassesSelector = this.bagEditor.options.renderingClasses
                .map(className => `.${className}`).join(", ");
            this.bagFieldObservers = [];

            $bagFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.bagEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $bagFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    //  `_pauseBagFieldObservers(field)` call.
                    this.__bagFieldObserversToPause = this.bagFieldObservers.filter(
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.bagEditor.options.renderingClasses);
                    }
                    const html = $(fieldNodeClone).html();
                    this.bagEditor.withoutRollback(() => {
                ...
                this.bagFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
     ...
     */
    _pauseBagFieldObservers() {
        // TODO adapt in master: remove this and directly exclude observers with
        // targets inside the current field (we use `this.bagFieldObservers`
        // as fallback for compatibility here).
        const fieldObserversData = this.__bagFieldObserversToPause || this.bagFieldObservers;
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.bagEditor.document.getSelection().getRangeAt(0).startContainer;
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.bagEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.bagEditor.document, 'a');
        ...
        if (this.state.showSnippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.bagEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.bagEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.bagEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.bagEditor.editable,
                        link,
                ...
                this.bagEditor.document.removeEventListener('click', this._onClick, true);
                ...
                this.bagEditor.document.addEventListener('click', this._onClick, true);
                document.addEventListener('click', this._onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.bagEditor.historySize() - 1;
            this.bagEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.bagEditor.historyUnpauseSteps();
                    this.bagEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt', options={}) {
        const restore = preserveCursor(this.bagEditor.document);
        const params = {
            insert: content => {
                this.bagEditor.historyPauseSteps();
                const insertedNodes = this.bagEditor.execCommand('insert', content);
                this.bagEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.bagEditor.editable.parentElement;
        ...
        if (mode === 'alternatives' || mode === 'translate') {
            params.originalText = this.bagEditor.document.getSelection().toString() || '';
        }
        if (mode === 'translate') {
            params.language = options.language;
        }
        this.bagEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.state.showSnippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.mutex.exec(() => {
                this.bagEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.bagEditor.execCommand('unlink');
        }
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.bagEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.bagEditor.document);

        const editable = BagEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.bagEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.bagEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.bagEditor.execCommand('insert', str);
                },
            }, {
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.bagEditor.unbreakableStepUnactive();
            ...
            if (params.node.matches(".oe_unremovable")) {
                ...
                this.bagEditor.historyStep();
                this.bagEditor.observerUnactive("unremovable");
                element.classList.add("oe_unremovable");
                this.bagEditor.observerActive("unremovable");
            } else {
                this.bagEditor.historyStep();
            }

            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.bagEditor.editable.focus();
        } else {
            const result = this.bagEditor.execCommand('insert', element);
            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.bagEditor.editable.focus();
            return result;
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.bagEditor.document, selector);
    }
    /**
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await bag.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.bagEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.bagEditor.document});
            const destroyTransfo = () => {
                $image.transfo('destroy');
                $(this.bagEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                this.bagEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
                imgTransformBtn.classList.toggle('active', $image[0].matches('[style*="transform"]'));
            };
            $(this.bagEditor.document).on('mouseup', mouseup);
            ...
            $(this.bagEditor.document).on('mousedown', mousedown);
            ...
            this.bagEditor.document.addEventListener('keydown', keydown);
        });
    ...
    _showImageCrop() {
        ...
        this.bagEditor.toolbarHide();
        $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.bagEditor.toolbarShow());
    }
    /**
     * @private
     * @param {jQuery} $
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.bagEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.bagEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.bagEditor.document.contains(element));
        ...
        if (coloredTds.length) {
            ...
            range.setEnd(...endPos(last));
                const { startContainer, startOffset, endContainer, endOffset } = getDeepRange(this.bagEditor.editable, { range });
            if (isSelForward) {
        ...
        this.bagEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.bagEditor.document.getSelection();
        if (!selection) return;
        const anchorNode = selection.anchorNode;
        if (isProtected(anchorNode)) {
            return;
        }
        if (this.bagEditor.document.querySelector(".transfo-container")) {
            return;
        }

        this.bagEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.bagEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.bagEditor.document.getSelection();
            const range = this.bagEditor.document.createRange();
        ...
        this.bagEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent BagEditor from controlling it as this is
    ...
    _getBannerCommand(title, emoji, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.bagEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.bagEditor.document, `
                ...
                `).childNodes[0];
                this.bagEditor.execCommand('insert', bannerElement);
                this.bagEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...
            this._getBannerCommand(_t('Banner Danger'), '❌', 'danger', 'fa-exclamation-circle', _t('Insert a danger banner'), 21),
            {
                ...
                isDisabled: () => !this.bagEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.bagEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.bagEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.bagEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (currentUser && currentUser.signature) {
                        this.bagEditor.execCommand('insert', parseHTML(this.bagEditor.document, currentUser.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.bagEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.bagEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.bagEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.bagEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.bagEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.bagEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.bagEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.bagEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.bagEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from bag field value
        // ZeroWidthSpace may be present from BagEditor edition process
    ...
    _onSelectionChange() {
        if (this.linkPopover && this.isSelectionInEditable()) {
            const selectionInLink = getInSelection(this.bagEditor.document, 'a') === this.linkPopover.target;
            const isVisible = this.linkPopover.el.offsetParent;
            if (
                isVisible &&
                (
                    (this.options.autohideToolbar && !this.bagEditor.document.getSelection().isCollapsed) ||
                    !selectionInLink
                )
            ) {
                this.linkPopover.hide();
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.bagEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.bagEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.bagEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.bagEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.bagEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.bagEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.bagEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.bagEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.bagEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.bagEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.bagEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.bagEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.bagEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.bagEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.bagEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the bag bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.bagEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.bagEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.bagEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.bagEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.bagEditor.historyResetFromSteps(steps, historyIds);
        this.bagEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.bagEditor && !this.bagEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.bagEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.bagEditor.historyGetBranchIds().join(',');
    ...
    _beforeAnyCommand() {
        ...
        for (const node of BagEditorLib.getTraversedNodes(this.$editable[0])) {
