...
import { Model, load } from "@bag/o-spreadsheet";
...
test("Bag formulas are migrated", () => {
    const data = {
        version: 16,
        sheets: [
            {
        ...
        sheets: [
            {
                cells: {
                    ...
                    A3: { content: `=bag.pivot("1","21/07/2021")` },
                    A4: { content: `=ODOO.PIVOT("1","test")` },
                    A5: { content: `=bag.pivot("1","21/07/2021")+"21/07/2021"` },
...
test("Bag version is exported", () => {
    const model = new Model();
    expect(model.exportData().bagVersion).toBe(ODOO_VERSION);
});
