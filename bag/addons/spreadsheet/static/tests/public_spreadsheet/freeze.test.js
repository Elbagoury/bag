...
import { registries } from "@bag/o-spreadsheet";
...
import { freezeBagData } from "@spreadsheet/helpers/model";
import { BagPivot, BagPivotRuntimeDefinition } from "@spreadsheet/pivot/bag_pivot";
...
test("bag pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeBagData(model);
...
test("bag pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeBagData(model);
...
test("bag charts are replaced with an image", async function () {
...
test("bag links are replaced with their label", async function () {
    const view = {
        name: "an bag view",
        viewType: "list",
        action: {
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: { content: "[menu_xml](bag://ir_menu_xml_id/test_menu)" },
                    A2: { content: "[menu_id](bag://ir_menu_id/12)" },
                    A3: { content: `[bag_view](bag://view/${JSON.stringify(view)})` },
                    A4: { content: "[external_link](https://bag.com)" },
    ...
    const frozenData = await freezeBagData(model);
    expect(frozenData.sheets[0].cells.A1.content).toBe("menu_xml");
    expect(frozenData.sheets[0].cells.A2.content).toBe("menu_id");
    expect(frozenData.sheets[0].cells.A3.content).toBe("bag_view");
    expect(frozenData.sheets[0].cells.A4.content).toBe("[external_link](https://bag.com)");
