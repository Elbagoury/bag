...
import { animationFrame, mockDate, mockTimeZone } from "@bag/hoot-mock";

import { DispatchResult, Model, helpers, tokenize } from "@bag/o-spreadsheet";
...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    const chartId = model.getters.getBagChartIds()[0];
    fieldMatching = model.getters.getBagChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("field matching is removed when an Bag chart is deleted", async function () {
    ...
    expect(model.getters.getBagChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
    expect(() => model.getters.getBagChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
...
test("Updating a non-bag pivot should not crash on global filter", async () => {
    const grid = {
