...
import { BagPivot } from "@spreadsheet/pivot/bag_pivot";
...
export async function insertPivotInSpreadsheet(model, pivotId, params) {
    ...
    if (!(ds instanceof BagPivot)) {
        throw new Error("The pivot data source is not an BagPivot");
    }
    await ds.load();
 ...
 * @returns {Promise<{ model: BagSpreadsheetModel, env: object, pivotId: string}>}
 */
