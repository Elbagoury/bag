 ...
 * @typedef {import("@spreadsheet/helpers/model").BagSpreadsheetModel} BagSpreadsheetModel
...
export function setupDataSourceEvaluation(model) {
    model.config.custom.bagDataProvider.addEventListener("data-source-updated", () => {
 ...
 * @returns {Promise<BagSpreadsheetModel>}
 */
export async function createModelWithDataSource(params = {}) {
    ...
    const model = new Model(params.spreadsheetData, {
        ...
        custom: {
            env,
            bagDataProvider: new BagDataProvider(env),
            ...config?.custom,
        },
    });
