...
import { parse, helpers, iterateAstNodes } from "@bag/o-spreadsheet";
import { isLoadingError } from "@spreadsheet/o_spreadsheet/errors";
import { loadBundle } from "@web/core/assets";
import { BagSpreadsheetModel } from "@spreadsheet/model";
import { BagDataProvider } from "@spreadsheet/data_sources/bag_data_provider";
...
import {
    ...
    isIrMenuXmlUrl,
} from "@spreadsheet/ir_ui_menu/bag_menu_link_cell";
...
export function createSpreadsheetModel({ env, data, revisions }) {
    const bagDataProvider = new BagDataProvider(env);
    const model = new BagSpreadsheetModel(data, { custom: { bagDataProvider } }, revisions);
    return model;
}

/**
 * @param {BagSpreadsheetModel} model
 */
export async function waitForBagSources(model) {
...
export async function waitForDataLoaded(model) {
    await waitForBagSources(model);
    const bagDataProvider = model.config.custom.bagDataProvider;
    if (!bagDataProvider) {
        return;
    }
    await new Promise((resolve, reject) => {
        function check() {
            ...
            if (isLoaded(model)) {
                bagDataProvider.removeEventListener("data-source-updated", check);
                resolve();
            }
        }
        bagDataProvider.addEventListener("data-source-updated", check);
        check();
    });
}

function containsLinkToBag(link) {
    if (link && link.url) {
        return (
 ...
 */
export async function freezeBagData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const [xc, cell] of Object.entries(sheet.cells)) {
            ...
            if (containsBagFunction(cell.content)) {
            ...
            if (containsLinkToBag(evaluatedCell.link)) {
        ...
        for (const figure of sheet.figures) {
            if (figure.tag === "chart" && figure.data.type.startsWith("bag_")) {
                ...
                const img = bagChartToImage(model, figure);
                figure.tag = "image";
                figure.data = {
                    path: img,
 ...
 * @returns {boolean}
 */
function containsBagFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
 ...
 * @returns {string}
 */
function bagChartToImage(model, figure) {
