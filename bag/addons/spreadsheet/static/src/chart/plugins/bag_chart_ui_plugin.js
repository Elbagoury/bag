...
export class BagChartUIPlugin extends BagUIPlugin {
    ...
    constructor(config) {
        ...
        globalFiltersFieldMatchers["chart"] = {
            ...
            waitForReady: () => this._getBagChartsWaitForReady(),
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getBagChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getBagChartIds().includes(cmd.id)) {
                        continue;
                    }
                    const dataSource = this.getChartDataSource(cmd.id);
                    if (
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getBagChartDataSourceId(chartId);
        return this.charts[dataSourceId];
    }

    /**
     * Get the callback used for onClick and onHover in an Bag Chart
     */
     ...
     * @return {Promise[]}
     */
    _getBagChartsWaitForReady() {
        return this.getters
            .getBagChartIds()
            .map((chartId) => this.getChartDataSource(chartId).loadMetadata());
    }

    _getBagChartDataSourceId(chartId) {
