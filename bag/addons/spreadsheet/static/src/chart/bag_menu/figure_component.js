...
patch(spreadsheet.components.FigureComponent.prototype, {
    ...
    async navigateToBagMenu() {
        const menu = this.env.model.getters.getChartBagMenu(this.props.figure.id);
        if (!menu) {
    ...
    get hasBagMenu() {
        return this.env.model.getters.getChartBagMenu(this.props.figure.id) !== undefined;
    },
    async onClick() {
        if (this.env.isDashboard() && this.hasBagMenu) {
            this.navigateToBagMenu();
        }
    },
});
