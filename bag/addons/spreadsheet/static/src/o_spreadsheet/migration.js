...
migrationStepRegistry.add("bag_migration", {
    versionFrom: "16.1",
    migrate(data) {
        return migrateBagData(data);
    },
});

function migrateBagData(data) {
    const version = data.bagVersion || 0;
    if (version < 1) {
    ...
    for (const sheet of data.sheets || []) {
        for (const xc in sheet.cells || []) {
            ...
            if (
                ...
                const tokens = tokenize(cell.content);
                /* given that bag.pivot.position is automatically set, we know that:
                ...
                3) there is only one bag.pivot.position per cell
                4) bag.pivot.position can only exist after the 3rd token and needs at least 7 tokens to be valid*/
                for (let i = 2; i < tokens.length - 7; i++) {
                    ...
                    if (
                        ...
                        // tokens[i-1] is the comma before bag.pivot.position
                        tokens[i] = order;
                        cell.content = tokensToString(tokens);
                    }
                }
...
export class BagVersion extends BagCorePlugin {
    static getters = /** @type {const} */ ([]);

    export(data) {
        data.bagVersion = ODOO_VERSION;
    }
}

corePluginRegistry.add("bagMigration", BagVersion);
