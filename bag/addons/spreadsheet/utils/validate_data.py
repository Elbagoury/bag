...
xml_id_url_prefix = "bag://ir_menu_xml_id/"

bag_view_link_prefix = "bag://view/"


def bag_charts(data):
    """return all bag chart definition in the spreadsheet"""
    figures = []
    for sheet in data.get("sheets", []):
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet.get("figures", [])
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("bag_")
        ]
    return figures
...
def bag_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like bag://view/{... view data...}
    """
    return [
        json.loads(url[len(bag_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(bag_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = bag_charts(data)
    bag_version = data.get("bagVersion", 1)
    if bag_version < 5:
...
def fields_in_spreadsheet(data):
    ...
    bag_pivots = (pivot for pivot in data.get("pivots", dict()).values() if pivot.get("type", "ODOO") == "ODOO")
    all_fields = chain(
        extract_fields(list_fields, data.get("lists", dict()).values()).items(),
        extract_fields(pivot_fields, bag_pivots).items(),
        extract_fields(chart_fields, bag_charts(data)).items(),
        extract_fields(bag_view_fields, bag_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartBagMenusReferences", {}).values()) | {
