...
from bag.sql_db import db_connect, TestCursor
from bag.tests import common
from bag.tests.common import BaseCase, HttpCase
from bag.tools.misc import config
...
class TestTestCursor(common.TransactionCase):
    def setUp(self):
        ...
        self.env = bag.api.Environment(self.cr, bag.SUPERUSER_ID, {})
    ...
    def test_interleaving(self):
        ...
        with self.assertLogs('bag.sql_db', level=logging.WARNING) as cm:
            a.close()
        [msg] = cm.output
        self.assertIn('WARNING:bag.sql_db:Found different un-closed cursor', msg)
