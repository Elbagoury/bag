...
@bag.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(bag.tests.TransactionCase):
...
class TestReportsRenderingCommon(bag.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        bag.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@bag.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@bag.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(bag.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
