...
from bag.tools import index_exists
from .account_edi_proxy_auth import BagEdiProxyAuth
    ...
    def _make_request(self, url, params=False):
        ...
        try:
            response = requests.post(
                ...
                auth=BagEdiProxyAuth(user=self)).json()
        ...
        if edi_mode == 'demo':
            ...
            if 'error' in response:
                if response['error'] == 'A user already exists with this identification.':
                    ...
                    # Bag side. We are for now forced to check the error message.
