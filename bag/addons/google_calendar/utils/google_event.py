...
class GoogleEvent(abc.Set):
    ...
    Inspired by Bag recordset, one instance can be a single Google event or a
    ...
    def bag_id(self, env):
        self.bag_ids(env)  # load ids
        return self._bag_id

    def _meta_bag_id(self, dbname):
        """Returns the Bag id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_bag_id' % dbname)
        if o_id:
            return int(o_id)

    def bag_ids(self, env):
        ids = tuple(e._bag_id for e in self if e._bag_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_bag_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_bag_ids_from_metadata(env, model)

        return tuple(e._bag_id for e in self)

    def _load_bag_ids_from_metadata(self, env, model):
        unsure_bag_ids = tuple(e._meta_bag_id(env.cr.dbname) for e in self)
        bag_events = model.browse(_id for _id in unsure_bag_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Bag id.
        # Therefore, we only consider Bag records without google id when trying
        # to match events.
        o_ids = bag_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            bag_id = e._meta_bag_id(env.cr.dbname)
            if bag_id in o_ids:
                e._events[e.id]['_bag_id'] = bag_id

    def _load_bag_ids_from_db(self, env, model):
        bag_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in bag_events}  # {google_id: bag_id}
        existing_google_ids = bag_events.mapped('google_id')
        for e in self:
            bag_id = mapping.get(e.id)
            if bag_id:
                e._events[e.id]['_bag_id'] = bag_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Bag.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Bag (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Bag) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Bag several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_bag_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.bag_ids(env)
        events.bag_ids(env)

        return self.filter(lambda e: e._bag_id)
    ...
    def get_bag_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.bag_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.bag_id(self.env)).base_event_id
