...
from bag import api, fields, models, _
from bag.modules.registry import Registry
from bag.tools import ormcache_context, email_normalize
from bag.osv import expression
from bag.sql_db import BaseCursor

from bag.addons.google_calendar.utils.google_event import GoogleEvent
from bag.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from bag.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Bag database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Bag.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Bag DB (_sync_google2bag).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_bag2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2bag(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        """Synchronize Google recurrences in Bag. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Bag
        :param write_dates: A dictionary mapping Bag record IDs to their write dates.
        :return: synchronized bag recurrences
        """
        ...
        bag_values = [
            dict(self._bag_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_bag = self.with_context(dont_notify=True)._create_from_google(new, bag_values)
        cancelled = existing.cancelled()
        cancelled_bag = self.browse(cancelled.bag_ids(self.env))

        # Check if it is a recurring event that has been rescheduled.
        # We have to check if an event already exists in Bag.
        # Explanation:
        ...
        if rescheduled_events:
            ...
            cancelled_bag += self.env['calendar.event'].search([('google_id', 'in', google_ids_to_remove)])

        cancelled_bag.exists()._cancel()
        synced_records = new_bag + cancelled_bag
        pending = existing - cancelled
        pending_bag = self.browse(pending.bag_ids(self.env)).exists()
        for gevent in pending:
            bag_record = self.browse(gevent.bag_id(self.env))
            if bag_record not in pending_bag:
                # The record must have been deleted in the mean time; nothing left to sync
                continue
            # Last updated wins.
            # This could be dangerous if google server time and bag server time are different
            updated = parse(gevent.updated)
            # Use the record's write_date to apply Google updates only if they are newer than Bag's write_date.
            bag_record_write_date = write_dates.get(bag_record.id, bag_record.write_date)
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not bag_record_write_date or updated >= pytz.utc.localize(bag_record_write_date):
                vals = dict(self._bag_values(gevent, default_reminders), need_sync=False)
                bag_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= bag_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Bag to Google
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from Bag to Google, False otherwise. """
        is_active_clause = (self._active_name, '=', True) if self._active_name else expression.TRUE_LEAF
    ...
    @api.model
    def _bag_values(self, google_event: GoogleEvent, default_reminders=()):
