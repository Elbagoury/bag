...
from bag import api, fields, models, tools, _
...
class Meeting(models.Model):
    ...
    @api.model
    def _bag_values(self, google_event, default_reminders=()):
        ...
        alarm_commands = self._bag_reminders_commands(reminder_command)
        attendee_commands, partner_commands = self._bag_attendee_commands(google_event)
    ...
    @api.model
    def _bag_attendee_commands(self, google_event):
        attendee_commands = []
        partner_commands = []
        ...
        if google_event.exists(self.env):
            event = google_event.get_bag_event(self.env)
        ...
        for bag_attendee in attendees_by_emails.values():
            ...
            email = tools.email_normalize(bag_attendee.email)
            if email not in emails and email != self.env.user.email:
                attendee_commands += [(2, bag_attendee.id)]
                partner_commands += [(3, bag_attendee.partner_id.id)]
        return attendee_commands, partner_commands

    @api.model
    def _bag_reminders_commands(self, reminders=()):
        commands = []
    ...
    def action_mass_archive(self, recurrence_update_setting):
        """ Delete recurrence in Bag if in 'all_events' or in 'future_events' edge case, triggering one mail. """
        self.ensure_one()
    ...
    def _google_values(self):
        ...
        values = {
            ...
            'extendedProperties': {
                'shared': {
                    '%s_bag_id' % self.env.cr.dbname: self.id,
                },
            },
            'reminders': {
                'overrides': reminders,
        ...
        if self.user_id and self.user_id != self.env.user and not bool(self.user_id.sudo().google_calendar_token):
            # The organizer is an Bag user that do not sync his calendar
            ...
            # the owner is not an Bag user: We don't store the real owner identity (mail)
            ...
            # values['extendedProperties']['private] should be used if the owner is not an bag user
