...
export class ClipboardPlugin extends Plugin {
    ...
    onCopy(ev) {
        ...
        const bagHtml = dataHtmlElement.innerHTML;
        const bagText = selection.textContent();
        ev.clipboardData.setData("text/plain", bagText);
        ev.clipboardData.setData("text/html", bagHtml);
        ev.clipboardData.setData("application/vnd.bag.bag-editor", bagHtml);
        if (commonAncestor && commonAncestor.nodeType === Node.ELEMENT_NODE) {
            this.dispatchTo("normalize_handlers", commonAncestor);
        }
    }
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePasteBagEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteBagEditorHtml(clipboardData) {
        const bagEditorHtml = clipboardData.getData("application/vnd.bag.bag-editor");
        if (bagEditorHtml) {
    ...
    onDragStart(ev) {
        if (ev.target.nodeName === "IMG") {
            ...
            ev.dataTransfer.setData(
                "application/vnd.bag.bag-editor-node",
    ...
    async onDrop(ev) {
        ...
        const imageNodeHTML = ev.dataTransfer.getData("application/vnd.bag.bag-editor-node");
        const image =
            imageNodeHTML &&
            this.dragImage &&
...
// @phoenix @todo: move to Bag plugin?
/**
