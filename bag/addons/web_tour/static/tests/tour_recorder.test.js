...
import { click, edit, keyDown, keyUp, press, queryOne } from "@bag/hoot-dom";
import { animationFrame, runAllTimers } from "@bag/hoot-mock";
import {
    contains,
    defineWebModels,
    mountWithCleanup,
...
test("Click on element with unique bag class", async () => {
...
test("Click on element with no unique bag class", async () => {
    await mountWithCleanup(
        `
        <div class="o_parent">
            <div class="o_child_1 click"></div>
...
test("Find the nearest bag class", async () => {
