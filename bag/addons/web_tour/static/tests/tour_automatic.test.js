...
import { afterEach, beforeEach, describe, expect, test } from "@bag/hoot";
import { animationFrame, queryFirst } from "@bag/hoot-dom";
import { advanceTime, runAllTimers } from "@bag/hoot-mock";
...
test("a failing tour logs the step that failed in run", async () => {
    ...
    await bag.startTour("tour2", { mode: "auto" }); // Use bag to run tour from registry because this is a test tour
...
test("a failing tour with disabled element", async () => {
    ...
    await bag.startTour("tour3", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
...
test("a failing tour logs the step that failed", async () => {
    ...
    await bag.startTour("tour1", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await waitForStep();
...
test("check tour with inactive steps", async () => {
    ...
    await bag.startTour("pipu_tour", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await waitForStep();
...
test("automatic tour with invisible element", async () => {
    ...
    await bag.startTour("tour_de_wallonie", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await advanceTime(10000);
...
test("automatic tour with alternative trigger", async () => {
    ...
    await mountWithCleanup(Root);
    await bag.startTour("tour_des_flandres", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await waitForStep();
...
test("check not possible to click below modal", async () => {
    ...
    await bag.startTour("tour_check_modal", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await advanceTime(10000);
...
test("a tour where hoot trigger failed", async () => {
    ...
    await bag.startTour("tour_hoot_failed", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    expect.verifySteps([
