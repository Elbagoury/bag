...
import { queryAll, queryFirst, queryOne } from "@bag/hoot-dom";
import { Component, useState, useExternalListener } from "@bag/owl";
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasBagClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasBagClass;
        ...
        const bagClass = [...currentElem.classList].find((c) => c.match(ODOO_CLASS_REGEX));
        if (bagClass) {
            currentPredicate = `.${bagClass}`;
            hasBagClass = true;
        }
