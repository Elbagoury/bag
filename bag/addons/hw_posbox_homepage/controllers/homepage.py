...
from bag.addons.hw_drivers.tools import helpers
from bag.addons.hw_drivers.main import iot_devices
from bag.addons.web.controllers.home import Home
from bag.addons.hw_drivers.connection_manager import connection_manager
from bag.tools.misc import file_path
from bag.addons.hw_drivers.server_logger import (
    check_and_update_bag_config_log_to_server_option,
    get_bag_config_log_to_server_option,
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('bag.addons.hw_posbox_homepage', "views")
...
class IotBoxOwlHomePage(Home):
    ...
    # ---------------------------------------------------------- #
    @http.route('/hw_posbox_homepage/restart_bag_service', auth='none', type='http', cors='*')
    def bag_service_restart(self):
        helpers.bag_restart(0)
        return json.dumps({
            'status': 'success',
            'message': 'Bag service restarted',
        })

    @http.route('/hw_posbox_homepage/iot_logs', auth='none', type='http', cors='*')
    def get_iot_logs(self):
        with open("/var/log/bag/bag-server.log", encoding="utf-8") as file:
            return json.dumps({
                'status': 'success',
                'logs': file.read(),
            })
    ...
    def get_homepage_data(self):
        ...
        return json.dumps({
            ...
            'server_status': helpers.get_bag_server_url() or 'Not Configured',
    ...
    def get_version_info(self):
        ...
        return json.dumps({
            ...
            'bagIsUpToDate': current_commit == last_available_commit or not bool(helpers.get_bag_server_url()),
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "Bag's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_bag_server_url(),
            'is_log_to_server_activated': get_bag_config_log_to_server_option(),
            'root_logger_log_level': self._get_logger_effective_level_str(logging.getLogger()),
            'bag_current_log_level': self._get_logger_effective_level_str(logging.getLogger('bag')),
    ...
    def update_wifi(self, essid, password, persistent=False):
        ...
        res_payload = {
            ...
            'server': {
                ...
                'message': 'Redirect to Bag Server' if server else 'Redirect to IoT Box'
            }
        }
    ...
    def connect_to_bag_server(self, token=False, iotname=False):
        if token:
            try:
    ...
    def update_log_level(self, name, value):
        ...
        if name == 'log-to-server':
            check_and_update_bag_config_log_to_server_option(value)

        name = name[len(IOT_LOGGING_PREFIX):]
        if name == 'root':
            self._update_logger_level('', value, AVAILABLE_LOG_LEVELS)
        elif name == 'bag':
            self._update_logger_level('bag', value, AVAILABLE_LOG_LEVELS)
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) Bag's configuration and logger to the given logger_name to the given level.
        ...
        if not IS_NEW_LEVEL_PARENT:
            ...
            _logger.debug('Adding to bag config log_handler: %s', new_entry)
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get Bag Iot logger given an IoT handler name
        ...
        """
        bag_addon_handler_path = helpers.compute_iot_handlers_addon_name(handler_folder_name, handler_name)
        return bag_addon_handler_path in logging.Logger.manager.loggerDict and \
               logging.getLogger(bag_addon_handler_path)
