...
from bag import http, models, fields, _
from bag.exceptions import AccessError, UserError
from bag.http import request, SessionExpiredException
from bag.osv import expression
from bag.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from bag.addons.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
from bag.addons.base.models.ir_qweb import QWebException
from bag.addons.portal.controllers.portal import pager as portal_pager
from bag.addons.portal.controllers.web import Home
from bag.addons.web.controllers.binary import Binary
from bag.addons.web.controllers.session import Session
from bag.addons.website.tools import get_base_domain
from bag.tools.json import scriptsafe as json
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/bag?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_info(self, **kwargs):
        ...
        values = {
            'apps': apps,
            'l10n': l10n,
            'version': bag.service.common.exp_version()
        }
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/bag/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/bag/ir.ui.view/{page.get('view_id')}")
