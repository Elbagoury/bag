...
class TestWebsiteAssets(bag.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/bag')
        check_asset()
        self.url_open(domain_2 + '/bag')
        check_asset()
        self.url_open(domain_1 + '/bag')
        check_asset()
        self.url_open(domain_2 + '/bag')
...
class TestWebAssets(bag.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with bag.tools.mute_logger('bag.addons.web.controllers.binary'):
            self.assertEqual(
