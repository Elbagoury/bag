...
import bag.http
from bag.tools.misc import hmac, DotDict, frozendict
...
def MockRequest(
    ...
    from bag.tests.common import HttpCase  # noqa: PLC0415
    ...
    if HttpCase.http_port():
        ...
        base_url = f"http://{HOST}:{bag.tools.config['http_port']}"
    request = Mock(
        # request
        httprequest=Mock(
            host='localhost',
        ...
        type='http',
        future_response=bag.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            bag.http.get_default_session(),
            sale_order_id=sale_order_id,
            website_sale_current_pl=website_sale_current_pl,
            context={'lang': ''},
        ),
        geoip=bag.http.GeoIP('127.0.0.1'),
    ...
    if country_code:
        request.geoip._city_record = bag.http.geoip2.models.City({'country': {'iso_code': country_code}})
    ...
    #
    #   router = bag.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        ...
        s.callback(bag.http._request_stack.pop)
        s.enter_context(patch('bag.http.root.get_db_router', router))
