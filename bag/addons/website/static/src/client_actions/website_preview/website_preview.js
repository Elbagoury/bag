...
export class WebsitePreview extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            if (!session.website_bypass_domain_redirect // Used by the Bag support (bugs to be expected)
                    && this.websiteDomain
                ...
                this.dialogService.add(WebsiteDialog, {
                    ...
                    onClose: () => {
                        window.location.href = `${encodeURI(this.websiteDomain)}/bag/action-website.website_preview?path=${encodedPath}&website_id=${encodeURIComponent(this.websiteId)}`;
                    }
                });
            } else {
        ...
        onMounted(() => {
            ...
            // BagFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('BagFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        });
        ...
        /**
         * This removes the 'Bag' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('BagFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('BagFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.websiteService.websiteRootInstance = undefined;
            this.iframe.el.addEventListener('BagFrameContentLoaded', resolve, { once: true });
            if (url) {
    ...
    _isTopWindowURL({ host, pathname }) {
        const backendRoutes = ['/web', '/web/session/logout', '/bag'];
    ...
    /**
     * This replaces the browser url (/bag/action-website...) with
    ...
    _replaceBrowserUrl() {
        if (!wUtils.isHTTPSorNakedDomainRedirection(this.iframe.el.contentWindow.location.origin, window.location.origin)) {
            ...
            // trigger a traceback.
            history.replaceState(history.state, document.title, '/bag');
            return;
        }
    ...
    _onKeydownRefresh(ev) {
        ...
        redirect(`/bag/action-website.website_preview?path=${encodeURIComponent(path)}${debugMode}`);
    }
}
