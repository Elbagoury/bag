...
import * as BagEditorLib from "@web_editor/js/editor/bag-editor/src/utils/utils";
import { Component, onMounted, onWillStart, useEffect, useRef, useState } from "@bag/owl";
...
const getTraversedNodes = BagEditorLib.getTraversedNodes;
...
export class WebsiteSnippetsMenu extends weSnippetEditor.SnippetsMenu {
    ...
    _getDragAndDropOptions(options = {}) {
        // TODO: This is currently not in use by Bag's D&D
        // There is currently no way in Bag D&D to offset the edge scrolling.
     ...
     */
    _getSelection() {
        return this.options.wysiwyg.bagEditor.document.getSelection();
    }
    /**
     * @override
     */
    _addToolbar() {
        ...
        this.options.wysiwyg.bagEditor.computeFontSizeSelectorValues();
    }
    /**
    * @override
    */
    ...
    _handleTextOptions(classSelector, optionClassList, textOptionsPostActivate = () => {}) {
        ...
        if (selectedTextEl) {
            const restoreCursor = BagEditorLib.preserveCursor(this.$body[0].ownerDocument);
            ...
            this.options.wysiwyg.bagEditor.historyResetLatestComputedSelection();
