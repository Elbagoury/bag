...
import { isMediaElement } from '@web_editor/js/editor/bag-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@bag/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.bagEditor.options.filterMutationRecords;
        Object.assign(this.bagEditor.options, {
            /**
             * @override
             */
        ...
        // Disable BagEditor observer's while setting up classes
        this.bagEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with bagEditor
        // observer) as the widgets might trigger DOM mutations.
        this._setObserver();
        this.bagEditor.observerActive();
    }
    /**
    ...
    async destroy() {
        ...
        const mod = await bag.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await bag.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.bagEditor.filterMutationRecords(records);
        ...
        this.bagEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.bagEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by BagEditor when text is entered.
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await bag.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    async _onSaveRequest(event) {
        ...
        if (event.data.reload || event.data.reloadEditor) {
            ...
            callback = () => redirect(`/bag/action-website.website_preview?website_id=${encodeURIComponent(websiteId)}&path=${currentPath}&enable_editor=1`);
