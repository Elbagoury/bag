...
copy_raspios () {
    ...
    wget -c 'https://nightly.bag.com/master/iotbox/iotbox-latest.zip' -O raspios/iotbox-latest.zip
    wget -c 'https://nightly.bag.com/master/iotbox/SHA1SUMS.txt' -O raspios/SHA1SUMS.txt
    cd raspios/
...
copy_iot () {
    ...
    # Modify startup
    mkdir -v bag
    mount -v "${PART_IOTBOX_ROOT}" bag
    cp -v /home/pi/upgrade.sh bag/home/pi/
    NBR_LIGNE=$(sed -n -e '$=' bag/etc/rc.local)
    sed -ie "${NBR_LIGNE}"'i\. /home/pi/upgrade.sh; clean_local' bag/etc/rc.local
    find /home/pi/config -maxdepth 1 -type f ! -name ".*" -exec cp {} bag/home/pi/ \;
