...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ "${ESSID}" ] && [ "${PASSWORD}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/bag/addons/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		service bag restart # As this file is executed on boot, this line is responsible for restarting bag service on reboot
	fi
# wired
else
	killall nginx
