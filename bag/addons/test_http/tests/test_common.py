...
import bag
from bag.http import Session
from bag.addons.base.tests.common import HttpCaseWithUserDemo
from bag.tools.func import lazy_property
from bag.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(bag.http.Application, 'session_store', session_store)
        cls.classPatch(bag.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(bag.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        bag.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('bag.http.db_list') as db_list, \
             patch('bag.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('bag.http.db_list') as db_list, \
             patch('bag.http.db_filter') as db_filter, \
             patch('bag.http.Registry') as Registry:
