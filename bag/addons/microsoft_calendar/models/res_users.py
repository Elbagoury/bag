...
import logging
import requests
from bag.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
...
from bag.addons.microsoft_account.models import microsoft_service
from bag.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Bag
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2bag(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Bag -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_bag2microsoft()
        ...
        (events - synced_events)._sync_bag2microsoft()
        self.sudo().microsoft_last_sync_date = fields.datetime.now()
