...
from bag.tests.common import HttpCase
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Bag event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Bag events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Bag events
        # -----------------------------------------------------------------------------------------

        self.expected_bag_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_bag_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_bag_recurrency_events_from_outlook = [
            {
    ...
    def sync_bag_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Bag. A trick for deactivating the checking
    ...
    def assert_bag_event(self, bag_event, expected_values):
        """
        Assert that an Bag event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        bag_event_values = bag_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(bag_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, bag_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_bag_recurrence(self, bag_recurrence, expected_values):
        """
        Assert that an Bag recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        bag_recurrence_values = bag_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, bag_recurrence_values.get(k), msg=f"'{k}' mismatch")
