...
from bag import exceptions
from bag.addons.mail.models.mail_message import Message
from bag.addons.mail.models.mail_thread import MailThread
from bag.addons.mail.tests.common import mail_new_test_user, MailCommon
from bag.addons.test_mail.data import test_mail_data
from bag.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE, THAI_EMAIL_WINDOWS_874
from bag.addons.test_mail.models.test_mail_models import MailTestGateway, MailTestGatewayGroups, MailTestTicket
from bag.sql_db import Cursor
from bag.tests import tagged, RecordCapturer
from bag.tools import mute_logger
from bag.tools.mail import email_split_and_format, formataddr
...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs('bag.addons.mail.models.mail_thread', level="WARNING") as capture:
        ...
        self.assertEqual(capture.output, [
            ("WARNING:bag.addons.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        incoming_bounce = self.format(
            ...
            email_from='MAILER-DAEMON@mailserver.bag.com (Mail Delivery System)',
            email_to='bounce@xxx.bag.com',
            delivered_to='bounce@xxx.bag.com'
        )
...
class MailGatewayCommon(MailCommon):
    ...
    def _reinject(self, force_msg_id=False, debug_log=False):
        ...
        if mail["headers"].get("X-Bag-Message-Id"):
            extra += f'\nX-Bag-Message-Id: {mail["headers"]["X-Bag-Message-Id"]}'
...
class TestMailgateway(MailGatewayCommon):
    ...
    @mute_logger('bag.addons.mail.models.mail_thread', 'bag.models')
    ...
    @mute_logger('bag.addons.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not bagbot:
        ...
        bagbot = self.env.ref('base.partner_root')
        bagbot.active = True
        bagbot.email = 'bagbot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, bagbot.email_formatted, f'groups@{self.alias_domain}',
                subject='Bagbot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, bagbot)
        self.assertEqual(record4.message_ids[0].email_from, bagbot.email_formatted)
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: bagbot -> no follower')
    ...
    @mute_logger('bag.addons.mail.models.mail_mail', 'bag.addons.mail.models.mail_thread', 'bag.models.unlink', 'bag.tests')
    ...
    @mute_logger('bag.addons.mail.models.mail_mail', 'bag.addons.mail.models.mail_thread', 'bag.models')
    def test_message_route_alias_owner_author_notify(self):
        ...
        self.assertFalse(self.user_root.active, 'notification logic relies on bagbot being archived')
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'Bagbot should be creating the message')
    ...
    @mute_logger('bag.addons.mail.models.mail_thread', 'bag.models.unlink', 'bag.addons.mail.models.mail_mail')
    ...
    @mute_logger('bag.addons.mail.models.mail_thread', 'bag.addons.mail.models.mail_mail', 'bag.models.unlink')
    ...
    @mute_logger('bag.addons.mail.models.mail_thread', 'bag.addons.mail.models.mail_mail', 'bag.models', 'bag.sql_db')
    def test_message_process_alias_config_invalid_defaults(self):
        ...
        with self.mock_mail_gateway(), patch('bag.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
        ...
        with self.assertRaises(Exception), patch('bag.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
                                                 autospec=True) as _alias_bounce_incoming_email_mock:
    ...
    @mute_logger('bag.addons.mail.models.mail_thread', 'bag.models', 'bag.tests')
    ...
    @mute_logger('bag.addons.mail.models.mail_thread', 'bag.models.unlink')
    ...
    @mute_logger('bag.addons.mail.models.mail_thread', 'bag.models.unlink', 'bag.addons.mail.models.mail_mail', 'bag.tests')
    def test_message_process_external_notification_reply(self):
        ...
        self.assertFalse(new_msg.is_internal, "Responses to messages sent by bagbot should always be public.")
        ...
        new_msg = self.test_record.message_ids[0]
        self.assertTrue(new_msg.is_internal, "Responses to messages sent by anyone but bagbot should keep"
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by bag should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        bag should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        ...
        # Bag 10 update: compat mode has been removed and should not work anymore
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Bag is still capable of decoding it.
...
class TestMailGatewayLoops(MailGatewayCommon):
    ...
    @mute_logger('bag.addons.mail.models.mail_mail', 'bag.addons.mail.models.mail_thread')
    ...
    def test_routing_loop_auto_notif(self):
        """ Test Bag servers talking to each other """
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.other_partner.email_formatted,
    ...
    def test_routing_loop_follower_alias(self):
        ...
        # simulate this email coming back to the same Bag server -> msg_id is
