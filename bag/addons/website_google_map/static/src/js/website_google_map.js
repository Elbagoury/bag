...
function initialize_map() {
    ...
    var map = new google.maps.Map(document.getElementById('bag-google-map'), {
        zoom: 1,
    ...
    async function initializeMarkers(bagPartnerData) {
        // Create the markers and cluster them on the map
        if (bagPartnerData) { /* bag_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < bag_partner_data.counter; i++) {
                markerPromises.push(set_marker(bag_partner_data.partners[i]));
            }
    ...
    initializeMarkers(bag_partner_data);
