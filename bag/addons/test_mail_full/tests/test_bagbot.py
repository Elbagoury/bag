...
from bag.addons.test_mail.tests.common import TestRecipients
from bag.tests import tagged
from bag.tools import mute_logger


@tagged("bagbot")
class TestBagbot(MailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestBagbot, cls).setUpClass()
        cls.test_record = cls.env['mail.test.simple'].with_context(cls._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        cls.bagbot = cls.env.ref("base.partner_root")
        ...
        cls.bagbot_ping_body = f'<a href="http://bag.com/bag/res.partner/{cls.bagbot.id}" class="o_mail_redirect" data-oe-id="{cls.bagbot.id}" data-oe-model="res.partner" target="_blank">@BagBot</a>'
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('bag.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_bagbot()
        bagbot = self.env.ref("base.partner_root")
        bagbot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', bagbot.id)])
        self.assertEqual(len(bagbot_in_fetch_listeners), 1, 'bagbot should appear only once in channel_fetch_listeners')

    @mute_logger('bag.addons.mail.models.mail_mail')
    def test_bagbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.bagbot_ping_body, 'partner_ids': [self.bagbot.id, self.user_admin.partner_id.id]})
        ...
        # Bagbot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.bagbot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last bagbot message instead
            sender=self.bagbot,
            answer=("@BagBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.bagbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.bagbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('bag.addons.mail.models.mail_mail')
    def test_bagbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @bagbot right now", 'partner_ids': []})
        self.assertNextMessage(
