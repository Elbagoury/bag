...
from bag.addons.hw_drivers.websocket_client import WebsocketClient
...
class Manager(Thread):
    ...
    def send_alldevices(self, iot_client=None):
        """
        This method send IoT Box and devices information to Bag database
        """
        if self.server_url:
    ...
    def run(self):
        """Thread that will load interfaces and drivers and contact the bag server with the updates"""
        self.server_url = helpers.get_bag_server_url()
