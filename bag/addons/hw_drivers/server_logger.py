...
from bag.netsvc import DBFormatter

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in bag.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, bag_server_url, active):
        """
        :param bag_server_url: Bag Server URL
        """
        super().__init__()
        self._bag_server_url = bag_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._bag_server_url:
    ...
    def _periodic_flush(self):
        bag_session = requests.Session()
        while self._bag_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(bag_session)

    def _flush_logs(self, bag_session):
        ...
        if not self._bag_server_url or queue_size == 0:
            return
        try:
            bag_session.post(
                self._bag_server_url + '/iot/log',
                data=empty_queue(),
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_bag_server_url(), get_bag_config_log_to_server_option())
...
# Set it in the 'root' logger, on which every logger (including bag) is a child
