...
export function usePartnerAutocomplete() {
    const keepLastBag = new KeepLast();
    ...
    async function autocomplete(value) {
        ...
        return new Promise((resolve, reject) => {
            const bagPromise = getBagSuggestions(value, isVAT).then((suggestions) => {
                bagSuggestions = suggestions;
            });
            ...
            const concatResults = () => {
                // Add Clearbit result with Bag result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    const websites = bagSuggestions.map((suggestion) => {
                    ...
                    clearbitSuggestions.forEach((suggestion) => {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            bagSuggestions.push(suggestion);
                        }
                    });
                }

                bagSuggestions = bagSuggestions.filter((suggestion) => {
                    return !suggestion.ignored;
                });
                bagSuggestions.forEach((suggestion) => {
                    delete suggestion.ignored;
                });
                return resolve(bagSuggestions);
            };

            whenAll([bagPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    }
    ...
    /**
     * Use Bag Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
     * @private
     */
    async function getBagSuggestions(value, isVAT) {
        ...
        const suggestions = await keepLastBag.add(prom);
