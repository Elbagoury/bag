...
import bag
from bag.modules import get_modules
from bag.modules.registry import Registry
from bag.release import nt_service_name
from bag.tools import config
from bag.tools.cache import log_ormcache_stats
from bag.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if bag.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'bag.service.http.request.%s' % (me.ident,)
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in bag.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="bag.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from bag.addons.base.models.ir_cron import ir_cron
        conn = bag.sql_db.db_connect('postgres')
        with conn.cursor() as cr:
            ...
            while True:
                ...
                registries = bag.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(bag.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="bag.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from bag.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('bag: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(bag.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = bag.service.db.list_dbs(True)
        return db_names
    ...
    def start(self):
        ...
        dbconn = bag.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(bag.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            bag.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if bag.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from bag.tests import loader  # noqa: PLC0415
    from bag.tests.suite import BagSuite  # noqa: PLC0415
            ...
            if config['test_enable']:
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = bag.api.Environment(cr, bag.SUPERUSER_ID, {})
...
def start(preload=None, stop=False):
    """ Start the bag http server and cron processor.
    """
    global server
    ...
    if bag.evented:
        server = GeventServer(bag.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(bag.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Bag is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(bag.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not bag.evented:
        if inotify:
