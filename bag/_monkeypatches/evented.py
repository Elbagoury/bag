...
It will initialize the `bag.evented` variable.
"""
import bag
import sys
...
def patch_evented():
    if bag.evented or not (len(sys.argv) > 1 and sys.argv[1] == 'gevent'):
        return
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in bag/debian/copyright)
        while 1:
            state = conn.poll()
