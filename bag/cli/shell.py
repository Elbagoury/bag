...
class Shell(Command):
    """Start bag in an interactive shell"""
    ...
    def init(self, args):
        ...
        bag.cli.server.report_configuration()
        bag.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = bag.api.Environment(cr, uid, {})['res.users'].context_get()
                env = bag.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
