...
import bag
from bag.modules import get_modules, get_module_path, initialize_sys_path
...
ODOO_HELP = """\
Bag CLI, use '{bag_bin} --help' for regular server options.

Available commands:
    {command_list}

Use '{bag_bin} <command> --help' for individual command help."""
...
def main():
    ...
    if len(args) > 1 and args[0].startswith('--addons-path=') and not args[1].startswith("-"):
        ...
        bag.tools.config._parse_config([args[0]])
        args = args[1:]
