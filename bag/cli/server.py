...
__author__ = bag.release.author
__version__ = bag.release.version

# Also use the `bag` logger for the main script.
_logger = logging.getLogger('bag')

re._MAXCACHE = 4096  # default is 512, a little too small for bag
...
def report_configuration():
    ...
    """
    config = bag.tools.config
    _logger.info("Bag version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', bag.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = bag.tools.config
    if not bag.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = bag.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            bag.tools.translate.trans_export(config["language"],
...
def import_translation():
    ...
    registry = bag.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = bag.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    bag.tools.config.parse_config(args, setup_logging=True)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                bag.service.db._create_empty_database(db_name)
            ...
            except bag.service.db.DatabaseExists:
                pass
    ...
    setup_pid_file()
    rc = bag.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the bag server (default command)"""
    def run(self, args):
        bag.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)
