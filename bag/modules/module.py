...
import bag
import bag.tools as tools
import bag.release as release
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `bag.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^bag\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("bag.addons.base.maintenance.migrations", "bag.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``bag.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook bag.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in bag.addons.__path__:
        bag.addons.__path__.append(dd)

    # hook bag.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(ad.strip()))
        if ad not in bag.addons.__path__:
            bag.addons.__path__.append(ad)

    # hook bag.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in bag.addons.__path__ and os.path.isdir(base_path):
        bag.addons.__path__.append(base_path)

    # hook bag.upgrade on upgrade-path
    from bag import upgrade
    ...
    # create decrecated module alias from bag.addons.base.maintenance.migrations to bag.upgrade
    spec = importlib.machinery.ModuleSpec("bag.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["bag.addons.base.maintenance"] = maintenance_pkg
    sys.modules["bag.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to bag and "crm"-like to bag.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in bag.addons.__path__:
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(bag.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'bag.addons.{module_name}'
