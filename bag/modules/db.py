...
def is_initialized(cr):
    ...
    """
    return bag.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    try:
        f = bag.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with bag.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in bag.modules.get_modules():
        mod_path = bag.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = bag.modules.get_manifest(i)
...
def has_trigram(cr):
    ...
    pg_trgm module but any similar function will be picked by Bag.
