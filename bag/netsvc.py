...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "bag.netsvc.log is deprecated starting Bag 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('bag.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='bag.tools.image')
...
DEFAULT_LOG_CONFIGURATION = [
    'bag.http.rpc.request:INFO',
    'bag.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['bag:DEBUG', 'bag.sql_db:INFO', 'bag.http.rpc:DEBUG'],
    'debug_rpc': ['bag:DEBUG', 'bag.sql_db:INFO', 'bag.http.rpc.request:DEBUG'],
    'debug': ['bag:DEBUG', 'bag.sql_db:INFO'],
    'debug_sql': ['bag.sql_db:DEBUG'],
    'info': [],
    'runbot': ['bag:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['bag:WARNING', 'werkzeug:WARNING'],
    'error': ['bag:ERROR', 'werkzeug:ERROR'],
    'critical': ['bag:CRITICAL', 'werkzeug:CRITICAL'],
}
