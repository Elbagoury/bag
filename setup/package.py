...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'bag', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/bag && \
    groupadd -g %(group_id)s bag && \
    useradd -u %(user_id)s -g bag bag -d /var/lib/bag && \
    mkdir /data && \
    chown bag:bag /var/lib/bag /data
USER bag
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class BagTestTimeoutError(Exception):
    pass


class BagTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise BagTestError("Installation of package failed")
        else:
        ...
        raise BagTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/bag_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.bag_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'bag/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'bag-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER bag', DOCKERUSER)
    ...
    def run(self, cmd, build_dir, container_name, user='bag', exposed_port=None, detach=False, timeout=None):
    ...
    def test_bag(self):
        logging.info('Starting to test Bag install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'bag.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise BagTestTimeoutError('Bag pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise BagTestError('Error while installing/starting Bag after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'bag-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/bag-*.tar.gz' % self.args.build_dir)[0], '%s/bag_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/bag-*.zip' % self.args.build_dir)[0], '%s/bag_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s bag"',
            'su bag -s /bin/bash -c "python3 -m venv /var/lib/bag/bagvenv"',
            'su bag -s /bin/bash -c "/var/lib/bag/bagvenv/bin/python3 -m pip install --upgrade pip"',
            'su bag -s /bin/bash -c "/var/lib/bag/bagvenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su bag -s /bin/bash -c "/var/lib/bag/bagvenv/bin/python3 -m pip install /data/src/bag_{VERSION}.{TSTAMP}.tar.gz"',
            'su bag -s /bin/bash -c "createdb mycompany"',
            'su bag -s /bin/bash -c "/var/lib/bag/bagvenv/bin/bag -d mycompany -i base --stop-after-init"',
            'su bag -s /bin/bash -c "/var/lib/bag/bagvenv/bin/bag -d mycompany --pidfile=/data/src/bag.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'bag-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_bag()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/bag (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../bag_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'bag-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/bag_{VERSION}.{TSTAMP}_all.deb',
            'su bag -s /bin/bash -c "bag -d mycompany -i base --pidfile=/data/src/bag.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'bag-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_bag()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/bag/rpmbuild'
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/bag.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./bag-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/bag-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" /data/src/setup/rpm/bag.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/bag*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'bag-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/bag-*.noarch.rpm' % self.args.build_dir)[0], '%s/bag_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s bag"',
            'su bag -c "createdb mycompany"',
            'dnf install -d 0 -e 0 /data/src/bag_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su bag -s /bin/bash -c "bag -c /etc/bag/bag.conf -d mycompany -i base --stop-after-init"',
            'su bag -s /bin/bash -c "bag -c /etc/bag/bag.conf -d mycompany --pidfile=/data/src/bag.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'bag-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_bag()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'bag-rpm-createrepo-%s' % TSTAMP)
...
class DockerWine(Docker):
    ...
    def build(self):
        ...
        container_python = '/var/lib/bag/.wine/drive_c/bagbuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        cmds = [
            ...
            rf'wine "c:\nsis-3.10\makensis.exe" {nsis_args} "c:\bagbuild\server\setup\win32\setup.nsi"',
            rf'wine {container_python} -m pip list'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'bag-win-build-%s' % TSTAMP)
